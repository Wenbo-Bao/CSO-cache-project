...

{
  ...
  //CODE HERE
  if ((address > main_memory_size_in_bytes-1)||(address < 0)) { //lowest 5 bits should be 000000
    printf("Error: address must be between 0 and size_in_bytes - 1\n");
    exit(1);
  }
  ...
  //CODE HERE
  uint32_t* start = (u_int32_t *)(main_memory + ((address & CACHE_LINE_ADDRESS_MASK)>>2));
