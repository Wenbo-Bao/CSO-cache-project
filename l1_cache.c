		     ...
		     uint8_t control, uint32_t *read_data, uint8_t *status)
{
  ...
  //CODE HERE
  uint32_t cache_entry_index = (address & L1_ADDRESS_INDEX_MASK) >> L1_ADDRESS_INDEX_SHIFT;
  ...
  //CODE HERE
  uint32_t tag_bits = (address & L1_ADDRESS_TAG_MASK) >> L1_ADDRESS_TAG_SHIFT;
  ...
  //CODE HERE
  uint32_t world_offset = (address & L1_ADDRESS_WORD_OFFSET_MASK) >> L1_ADDRESS_WORD_OFFSET_SHIFT;
  ...
  //CODE HERE
  uint32_t* tag_of_entry = &(l1_cache[cache_entry_index].v_d_tag);
  // if ((tag_bits ^ (*tag_of_entry & L1_ENTRY_TAG_MASK))){
  // }
  // if(!((*tag_of_entry & L1_VBIT_MASK))){
  // }
  if ((tag_bits ^ (*tag_of_entry & L1_ENTRY_TAG_MASK)) || !((*tag_of_entry & L1_VBIT_MASK))){
    *status = *status & (~L1_HIT_STATUS_MASK);
    return;
  }
  ...
  *status = (*status | L1_HIT_STATUS_MASK);
  if (control & 0x1){
    *read_data =  l1_cache[cache_entry_index].cache_line[world_offset];
  }
  if (control & 0x2){
    * tag_of_entry = * tag_of_entry | L1_DIRTYBIT_MASK;
    l1_cache[cache_entry_index].cache_line[world_offset] = write_data;
  }
  return;
}
		    ...
		    uint8_t *status)
{
  ...
  if ((*tag_of_entry & L1_VBIT_MASK) && (*tag_of_entry & L1_DIRTYBIT_MASK)){
    *evicted_writeback_address =  (*tag_of_entry << L1_ADDRESS_TAG_SHIFT) | (cache_entry_index << L1_ADDRESS_INDEX_SHIFT);

    for (int i = 0; i < 16; i++){
      evicted_writeback_data[i] = l1_cache[cache_entry_index].cache_line[i];
    }
    *status = (*status | L1_HIT_STATUS_MASK);
  } else {
    *status = (*status & ~L1_HIT_STATUS_MASK);
  ...
  //CODE HERE
  for (int i = 0; i < 16; i++){
    l1_cache[cache_entry_index].cache_line[i] = write_data[i];
  }
  
  *tag_of_entry = *tag_of_entry & (~L1_DIRTYBIT_MASK);
  *tag_of_entry = *tag_of_entry | (L1_VBIT_MASK);
  *tag_of_entry = *tag_of_entry & (~L1_ENTRY_TAG_MASK);
  *tag_of_entry = *tag_of_entry | tag_bits;
