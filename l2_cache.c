		     ...
		     uint8_t control, uint32_t read_data[], uint8_t *status)
{
  ...
  //CODE HERE
  uint32_t cache_set_index = (address & L2_ADDRESS_INDEX_MASK) >> L2_ADDRESS_INDEX_SHIFT;
  ...
  for (int i  = 0; i < L2_LINES_PER_SET; i++){
    uint32_t* tagline = &(l2_cache[cache_set_index].lines[i].v_r_d_tag);
    if ((((*tagline & L2_ENTRY_TAG_MASK)^tag_bits) == 0) && (*tagline & L2_VBIT_MASK)){
      * tagline = * tagline | L2_RBIT_MASK;
      *status = (*status | L2_HIT_STATUS_MASK);
      if (control & 0x1){
        for(int j = 0; j < WORDS_PER_CACHE_LINE; j++){
          read_data[j] =  l2_cache[cache_set_index].lines[i].cache_line[j];
        }
      }
      if (control & 0x2){
         * tagline = * tagline | L2_DIRTYBIT_MASK;
        for(int k = 0; k < WORDS_PER_CACHE_LINE; k++){
          l2_cache[cache_set_index].lines[i].cache_line[k] =  write_data[k];
        }
      }
      return;
    }
...
void l2_insert_line(uint32_t address, uint32_t write_data[], 
  ...
  //CODE HERE
  uint32_t cache_set_index = (address & L2_ADDRESS_INDEX_MASK) >> L2_ADDRESS_INDEX_SHIFT;
  // printf("the set index is %d",cache_set_index);
  // L2_CACHE_ENTRY entries[L2_LINES_PER_SET] = (l2_cache[cache_set_index]).lines;
  //Extract from the address the tag bits. 
  //see l2_cache_access above.
  ...
  //LOOP STARTS HERE
       ...
       // CODE HERE
  for (int i  = 0; i < L2_LINES_PER_SET; i++){
      uint32_t* tag = &((l2_cache[cache_set_index]).lines[i].v_r_d_tag);
    if (!(*tag & L2_VBIT_MASK)){
      // printf("I am at zero");
      *status = *status & (~L2_HIT_STATUS_MASK);
      *tag = *tag | (L2_VBIT_MASK);
      *tag = *tag & (~L2_RBIT_MASK);
      *tag = *tag & (~L2_DIRTYBIT_MASK);
      for(int j = 0; j < WORDS_PER_CACHE_LINE; j++){
        l2_cache[cache_set_index].lines[i].cache_line[j] =  write_data[j];
      }
      *tag = *tag & (~L2_ENTRY_TAG_MASK);
      *tag = *tag | tag_bits;
      return;
    }
  }
  ...
  if (~r0_d0_index){
    // printf("I am at 1");
      uint32_t* tag = &((l2_cache[cache_set_index]).lines[r0_d0_index].v_r_d_tag);
      ...
      *tag = *tag & (~ L2_DIRTYBIT_MASK);
      for(int j = 0; j < WORDS_PER_CACHE_LINE; j++){
        l2_cache[cache_set_index].lines[r0_d0_index].cache_line[j] =  write_data[j];
      }
      *tag = *tag & (~L2_ENTRY_TAG_MASK);
      *tag = *tag | tag_bits;
      return;
    ...
    {
        uint32_t* tag = &((l2_cache[cache_set_index]).lines[r0_d1_index].v_r_d_tag);
      // printf("I am at 2");
        *status = (*status | L2_HIT_STATUS_MASK);
        *evicted_writeback_address = (*tag << L2_ADDRESS_TAG_SHIFT) | (cache_set_index << L2_ADDRESS_INDEX_SHIFT);
        *tag = *tag & (~L2_RBIT_MASK);
        *tag = *tag | (L2_VBIT_MASK);
        *tag = *tag & (~L2_DIRTYBIT_MASK);
        for(int j = 0; j < WORDS_PER_CACHE_LINE; j++){
          evicted_writeback_data[j] = l2_cache[cache_set_index].lines[r0_d1_index].cache_line[j];
          l2_cache[cache_set_index].lines[r0_d1_index].cache_line[j] =  write_data[j];
        }
        *tag = *tag & (~L2_ENTRY_TAG_MASK);
        *tag = *tag | tag_bits;
        return;
    ...
    {
      // printf("I am at 3");
        uint32_t* tag = &((l2_cache[cache_set_index]).lines[r1_d0_index].v_r_d_tag);
        *status = *status & (~L2_HIT_STATUS_MASK);
        *tag = *tag & (~L2_RBIT_MASK);
        *tag = *tag | (L2_VBIT_MASK);
        *tag = *tag & (~L2_DIRTYBIT_MASK);
        for(int j = 0; j < WORDS_PER_CACHE_LINE; j++){
          l2_cache[cache_set_index].lines[r1_d0_index].cache_line[j] =  write_data[j];
        }
        *tag = *tag & (~L2_ENTRY_TAG_MASK);
        *tag = *tag | tag_bits;
        return;
        ...
        // printf("I am at 4");
        uint32_t* tag = &((l2_cache[cache_set_index]).lines[0].v_r_d_tag);
        *status = (*status | L2_HIT_STATUS_MASK);
        *evicted_writeback_address = (*tag << L2_ADDRESS_TAG_SHIFT) | (cache_set_index << L2_ADDRESS_INDEX_SHIFT);
        *tag = *tag & (~L2_RBIT_MASK);
        *tag = *tag | (L2_VBIT_MASK);
        *tag = *tag & (~L2_DIRTYBIT_MASK);
        for(int j = 0; j < WORDS_PER_CACHE_LINE; j++){
          evicted_writeback_data[j] = l2_cache[cache_set_index].lines[0].cache_line[j];
          l2_cache[cache_set_index].lines[0].cache_line[j] =  write_data[j];
        }
        *tag = *tag & (~L2_ENTRY_TAG_MASK);
        *tag = *tag | tag_bits;
        return;
...
void l2_clear_r_bits(){
  for (int i = 0; i < L2_NUM_CACHE_SETS; i++){
    for (int j = 0; j < L2_LINES_PER_SET; j++){
      uint32_t* tag = &((l2_cache[i].lines)[j].v_r_d_tag);
      *tag = *tag & ~L2_RBIT_MASK;
    }
  }
}
